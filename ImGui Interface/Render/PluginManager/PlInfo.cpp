...
void ImGuiPlInfo::Load()
{
	...
	this->Plugin = SG::PluginManager->LoadPlugin(this->PluginName);
...
void ImGuiPlInfo::Unload()
{
	if (this->Plugin)
	{
		ImGuiInterface* pIFace = static_cast<ImGuiInterface*>(SG::ThisPlugin);
		pIFace->RemoveCallback(this->Plugin);

		SG::PluginManager->RequestShutdown(this->Plugin);
...
void ImGuiPlInfo::Reload()
{
	if (this->State == PluginState::Loaded)
	{
		...
		SG::PluginManager->RequestShutdown(this->Plugin);
		if (!(this->Plugin = SG::PluginManager->LoadPlugin(this->PluginName)))
...
{
	auto& props = static_cast<ImGuiInterface*>(SG::ThisPlugin)->GetPropManager();
